...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const { name, email, phone, location, experience, current_role,
            skills, why_bluwav, availability, linkedin, resume_note, role } = body;

    const adminHtml = `<h2 style="color:#002B49;font-family:sans-serif;">New Application — ${role}</h2>
      <table style="border-collapse:collapse;width:100%;font-family:sans-serif;font-size:14px;">
        ${row('Role', role)}${row('Name', name)}${row('Email', '<a href="mailto:'+email+'">'+email+'</a>')}
        ${row('Phone', phone)}${row('Location', location)}${row('Experience', experience)}
        ${row('Current Role', current_role)}${row('Skills', skills)}
        ${row('Why BluWav', why_bluwav)}${row('Availability', availability)}
        ${row('LinkedIn', linkedin)}${row('Resume', resume_note)}
      </table>`;
    ...
    const confirmHtml = `<div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:32px 24px;">
      ...
      <p style="color:#64748b;font-size:13px;">The BluWav Growth Team<br/>
      <a href="mailto:careers@bluwavgrowth.com" style="color:#FF5500;">careers@bluwavgrowth.com</a></p>
    </div>`;
    ...
    const e = await r1.text() + ' | ' + await r2.text();
    return new Response(JSON.stringify({ success: false, error: e }), { status: 500, headers: corsHeaders() });
